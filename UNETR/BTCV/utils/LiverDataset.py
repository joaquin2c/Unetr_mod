...
class LiverDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        #print("pre liver: ",np.shape(img))
        mask = []
        for i in range(self.num_classes):
            mask_pre=cv2.imread(os.path.join(self.mask_dir, str(i),img_id + self.mask_ext), cv2.IMREAD_GRAYSCALE)
            _ , mask_post= cv2.threshold(mask_pre,5,255,cv2.THRESH_BINARY)
            mask.append(mask_post[..., None])
        mask = np.dstack(mask)

        #print("size= ",np.shape(img))
